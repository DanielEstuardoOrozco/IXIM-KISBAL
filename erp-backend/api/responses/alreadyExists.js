 ...
 *     // -or-
 *     return res.alreadyExists(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function alreadyExists(optionalData) {
  ...
  var statusToSet = "ALREADY_EXISTS";
  // If no data was provided, use res.sendStatus().
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.alreadyExists()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.alreadyExists()` called with an Error:', optionalData);
