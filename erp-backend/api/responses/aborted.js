 ...
 * aborted.js
 *
 * 409 (Aborted) Response
 *
 * Example usage:
 * ```
 *     return res.aborted();
 *     // -or-
 *     return res.aborted(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function aborted(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.aborted()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.aborted()` called with an Error:', optionalData);
